...
#> Error in fct_reorder(str_trunc(class, 40), value): could not find function "fct_reorder"
```
...
res5 %>%
  ...
  labs(title = "Total texts per language in the HathiTrust bookworm",
       ...
       x = "Year", y = "")
#> Error: Problem with `mutate()` column `language`.
#> i `language = fct_lump_n(language, 10, w = value)`.
#> x could not find function "fct_lump_n"
```
