...
This package has opinionated defaults: if you don't do anything, it will assume you want to rsync and cache all your extracted features files to `getOption("hathiTools.ef.dir")`, which is just `r getOption("hathiTools.ef.dir")` unless you explicitly change it.You can, however, set your options differently, either by specifying `dir` explicitly in functions like `rsync_from_hathi()`, `cache_htids()`, `find_cached_htids()`, and `read_cached_htids()`, or by setting the `hathiTools.ef.dir` option directly (i.e., calling `options(hathiTools.ef.dir = "your_preferred_dir")`). 
