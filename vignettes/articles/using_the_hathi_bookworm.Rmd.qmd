...
res3 %>% filter(languages == "eng") %>% arrange(desc(value))
```
...
To get the metadata for all of these volumes at the same time, it is best to rsync them and cache them:

```{r}
rsync_from_hathi(res4[2:5, ])

cached_files <- cache_htids(res4[2:5, ])

cached_files

cached_files %>%
  filter(cache_format == "Metadata") %>%
  pull(local_loc) %>%
  map_df(read_rds)
```
