...
test_that("Can cache to text2vec.csv", {
  ...
  skip_on_ci()
  skip_if_offline()
  cache_htids(poetry$htid[1:10], dir = dir, cache_type = "text2vec")
...
test_that("Can cache to arrow", {
  ...
  skip_on_ci()
  skip_if_offline()
  cache_htids(poetry$htid[1:10], dir = dir, cache_type = "feather")
...
test_that("Can cache workset", {
  ...
  cache_htids(workset, dir = dir, cache_type = "feather")
  files <- fs::dir_ls(path = dir, recurse = TRUE, glob = "*.feather")
  expect_equal(length(files), 10)
  expect_snapshot(get_hathi_counts(poetry$htid[1], dir = dir, cache_type = "feather"))
})
